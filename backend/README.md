...
- `OPENAI_CHAT_MODEL`：仅 `/api/chat` 的上游模型；未设或空白时回退 `OPENAI_MODEL`，再回退 `gpt-4o-mini`
- `OPENAI_GENERATION_MODEL`：仅 `answer/generate` 触达上游时的模型；未设则回退 `OPENAI_MODEL`，再回退 `gpt-4o-mini`
- `OPENAI_MODEL`：chat 与解析的共用回退模型，不能覆盖已设置的 `OPENAI_CHAT_MODEL` / `OPENAI_GENERATION_MODEL`
